...
describe("join", () => {
    ...
    */
    ...
    it("table -> compound", async () => {
        const q = t1.join("NATURAL", u).noConstraint().selectStar().stringify();
    ...
    it("table -> compound -- USING", async () => {
        const q = t1.join("LEFT", u).using(["b"]).selectStar().stringify();
        expect(q).toMatchInlineSnapshot(
    ...
    it("select -> compound -- USING", async () => {
        const q = q1.join("LEFT", u).using(["b"]).selectStar().stringify();
        expect(q).toMatchInlineSnapshot(
    ...
    it("stringified select -> compound -- USING", async () => {
        const q = str1.join("LEFT", u).using(["b"]).selectStar().stringify();
        expect(q).toMatchInlineSnapshot(
            `SELECT * FROM (SELECT * FROM \`t1\`) AS \`t1\` LEFT JOIN (SELECT * FROM \`t1\` UNION ALL SELECT * FROM \`t2\`) AS \`u\` USING(\`b\`)`
        );
    });
    ...
    it("compound -> compound", async () => {
        const u1 = unionAll([q1, q3]).as("q1");

        const q = u1.join("LEFT", u).noConstraint().selectStar().stringify();
        expect(q).toMatchInlineSnapshot(
    ...
    it("compound -> compound -- USING", async () => {
        ...
        const q = u1.join("LEFT", u).using(["b"]).selectStar().stringify();
        expect(q).toMatchInlineSnapshot(
